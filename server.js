...
(function() {
  var KeyObject, Map, ObjectUser, async, battles, clone, counter_battle, fs, incrementAndGet, isGame, keys, locks, maps, mutex_battles, mysql, net, pool, sockets, sys, users_sockets, util;
  ...
  if (process.argv[2] === "S" || process.argv[2] === "s") {
    isGame = true;
  }
      ...
      return connection.query("SELECT o.color, o.id, om.posX, om.posY, om.admin FROM object_map om INNER JOIN object o ON o.id = om.id_obj WHERE om.id_map=1;", function(err, rows, fields) {
        ...
        svr = net.createServer(function(sock) {
          ...
          newConnectionWarningUser = function(ou) {
            ...
            if (isGame === true) {
              for (port in sockets) {
                if (port !== sock.remotePort) {
                  if (sockets[port]) {
                    sockets[port].write(msg);
                  }
                }
              }
            }
              ...
              for (j = 0, len1 = sockets.length; j < len1; j++) {
                ...
                if (port === receiver_id) {
                  ...
                  connection.query("UPDATE `users` SET `rollDice`=" + randomValue(1, 6) + "; WHERE `port`=" + receiver_id + ";", function(err, res) {
                    return sockets[port].write(ret);
                  });
                } else {
                  sockets[port].write(retOthers(port !== emisor_id ? sockets[port] !== sock ? sockets[port] ? sockets[port].write(data) : void 0 : void 0 : void 0));
                }
              }
            });
          };
          ...
          sock.on("data", function(data) {
            var Exception, d, id_battle, id_enemy, idx, insOrUpd, msg, port, posX, posY, ret;
              ...
              switch (d.Action) {
                ...
                case "okInitFight":
                  ...
                  if (fs.getPlayers() === 2) {
                    ...
                    battles[id_battle] = fs;
                    setTimeout((function() {
                      return sendDiePlayerAndWinnerToShow(fs.getIdEmisor()(fs.getIdReceiver()));
                    }), 1000);
                  }
                  return;
                case "fight":
                ...
                case "getObj":
                  ...
                  connection.query("UPDATE `object_map` SET `admin`='" + users_sockets[sock.remotePort] + "' WHERE `id`=" + d.Id_obj + " AND `id_map`=1;", function(err) {
                    ...
                    if (!err) {
                      ...
                      for (j = 0, len1 = sockets.length; j < len1; j++) {
                        port = sockets[j];
                        if (sockets[port] !== sock) {
                          if (sockets[port]) {
                            results1.push(sockets[port].write(data));
                          } else {
